...
int main()
{
  pthread_t thid, mythid, new_thread_for_task;
  int       result, my_thr_result;
  //
  ...
  my_thr_result = pthread_create (&new_thread_for_task, (pthread_attr_t *)NULL, mythread, NULL);
  ...
  printf("I created my thread, its id = %u\n", new_thread_for_task);

  a=a+1;
  
  printf("Calculation result after adding new thread= %d\n", a);
  //
